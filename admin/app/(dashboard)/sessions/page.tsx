  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">活跃会话</h1>
          <p className="text-slate-400 mt-1">实时监控进行中的充电会话（5 秒刷新）</p>
        </div>
      ...
      <Card className="bg-slate-800/80 backdrop-blur-sm border-slate-700">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-slate-400 text-center py-12">当前无进行中的充电会话</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-slate-300">
                <thead>
                  <tr className="border-b border-slate-700 text-slate-400">
                    <th className="text-left py-2 px-2">会话 ID</th>
                    <th className="text-left py-2 px-2">充电桩</th>
                    <th className="text-left py-2 px-2">用户</th>
                    <th className="text-left py-2 px-2">已充电量</th>
                    <th className="text-left py-2 px-2">功率</th>
                    <th className="text-left py-2 px-2">时长(分)</th>
                ...
                <tbody>
                  {sessions.map((s) => (
                    <tr key={s.id} className="border-b border-slate-700/50">
                      ...
                      <td className="py-2 px-2 text-xs">{s.user_id?.slice(0, 8) ?? '—'}…</td>
