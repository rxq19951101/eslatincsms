...
interface ActiveSession {
  ...
  charge_point_id: string; // Internal UUID used only for relations.
              ...
              <table data-testid="admin-sessions-table" className="w-full text-sm text-slate-300">
                ...
                <tbody>
                  {sessions.map((s) => {
                    ...
                    return (
                      <tr key={s.id} data-testid="admin-session-row" data-transaction-id={s.transaction_id} className="border-b border-slate-700/50">
                        <td className="py-2 px-2">{s.transaction_id ?? fallback}</td>
                        <td className="py-2 px-2 font-mono text-xs" data-testid="session-ocpp-identity">
                          {s.ocpp_identity?.trim() || fallback}
                        </td>
                        <td className="py-2 px-2">{formatDateTime(s.start_time, locale, fallback)}</td>
                        <td className="py-2 px-2">{formatMeasurement(s.energy_kwh, 2, 'kWh', fallback)}</td>
                        <td className="py-2 px-2">{formatMeasurement(s.power_kw, 1, 'kW', fallback)}</td>
                        <td className="py-2 px-2">{formatMeasurement(s.duration_minutes, 0, '', fallback)}</td>
                        <td className="py-2 px-2">
                          <Badge className={getStatusBadgeClass(s.status)}>{t(getStatusMessageKey(s.status))}</Badge>
                        </td>
                      </tr>
                    );
                  })}
